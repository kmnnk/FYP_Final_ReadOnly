...
from tensorflow.keras.layers import LeakyReLU, Conv2D, MaxPooling2D, Flatten, Dense, Conv2DTranspose, Flatten, Dense, UpSampling2D, Reshape, Dropout, BatchNormalization, Activation, Lambda, concatenate, Input
from tensorflow.keras.callbacks import EarlyStopping, ModelCheckpoint
...
def build_model(alpha1, alpha2, gamma, optimizer, n_layers, filter_num, conv_size, pool_size, dropout_rate, l2_f, use_bn, activation, dimred_act, kernel_initialiser, act_alpha=None, act_alpha_DR=None):
    ...
    inputs = Input((200, 200, 1024)) #change according to the input size, ie. for LDA and PCA the depth will be different
    #chnage this filter number based on the dimension required
    x = Conv2D(1, (1, 1), activation = dimred_act, kernel_initializer=kernel_initialiser)(inputs) #kernel intialiser can be used regardless or activation as just determining how to set the initial random weights
    ...
    # Middle
    x = Conv2D(filter_num * 2**n_layers, (conv_size, conv_size), activation=activation, padding='same', kernel_regularizer=l2(l2_f), kernel_initializer=kernel_initialiser)(x)
    if use_bn:
        x = BatchNormalization()(x)
    x = Conv2D(filter_num * 2**n_layers, (conv_size, conv_size), activation=activation, padding='same', kernel_regularizer=l2(l2_f), kernel_initializer=kernel_initialiser)(x)
    if use_bn:
    ...
    for i in range(n_layers-1, -1, -1):
        x = Conv2DTranspose(filter_num * 2**i, (conv_size, conv_size), strides=(pool_size, pool_size), padding='same', kernel_initializer=kernel_initialiser)(x)
        if use_bn:
            x = BatchNormalization()(x)
...
def objective(trial):
    #session clearing at the start, to account for latency
    K.clear_session()
    gc.collect()
    ...
    for fold_number, (train_index, test_index) in enumerate(kfold.split(X_train_val)):
        ...
        X_train_kv, X_val_kv, y_train_kv, y_val_kv = train_test_split(X_train1, y_train1, test_size=0.2, random_state=42)
        
        #usng data generator to retrieve the data according to the batch size to limit OOM
        ...
        checkpoint = ModelCheckpoint(f'/rds/general/user/nmk120/home/Mod_HypP/1_aug/model_trial_{trial.number}_fold_{fold_number}_act.h5', monitor='val_auc', verbose=1, save_best_only=True, mode='max')
        ...
        # history = model.fit(X_train_kv, y_train_kv, batch_size=batch_size, epochs=100, validation_data=(X_val_kv, y_val_kv), callbacks=[early_stopping, KerasPruningCallback(trial, 'val_auc'), checkpoint])
    ...
    return np.median(scores) #median for robustness against outliers
...
fig.write_image("/rds/general/user/nmk120/home/Mod_HypP/1_aug/optunaS_optimization_history_200t_fl_701515_NEW_act.png")
...
load_best_model = load_model(f'/rds/general/user/nmk120/home/Mod_HypP/1_aug/model_trial_{best_trial_number}_fold_{best_fold_in_best_trial}_act.h5', custom_objects={'focal_loss_fixed': focal_loss(alpha=[alpha1, alpha2], gamma=gamma)})
...
plt.xlabel('Epoch')
plt.legend(['Train', 'Validation'], loc='upper left')
plt.savefig('/rds/general/user/nmk120/home/Mod_HypP/1_aug/retrained_BM_perf_200t_act.png')
...
with open('/rds/general/user/nmk120/home/Mod_HypP/1_aug/optuna_mod_hyperParam_metrics_200t_fl_701515_NEW_act.txt', 'w') as f:
